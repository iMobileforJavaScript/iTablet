...
export default class NewDSet extends React.Component {
  ...
  createDataset = () => {
    ...
    (async function () {
      try {
        let dtname = await this.datasource.getAvailableDatasetName(this.state.name)
        let datasetVectorInfo = await new DatasetVectorInfo().createObjByNameType(dtname, this.type)
        let datasetVector = await this.datasource.createDatasetVector(datasetVectorInfo)
        if (datasetVector) {
          Toast.show('创建数据集成功')
          this.cb && this.cb()
          NavigationService.goBack(this.props.nav.routes[this.props.nav.routes.length - 2].key)
        } else {
          Toast.show('创建数据集失败')
        }
      } catch (e) {
  ...
  render() {
    return (
      <Container
        ...
        <BtnTwo style={{marginTop: scaleSize(80)}} text='确定' btnClick={this.createDataset} />
