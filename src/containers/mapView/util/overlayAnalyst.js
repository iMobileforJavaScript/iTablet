 ...
 */
async function analyst({workspace, method, dataset, targetDataset, parameter}) {
  try {
    ...
    switch (method) {
      case METHOD_CLIP:
        result = await overlay.clip(dataset, targetDataset, resultDataset, overlayParameter)
        break
      case METHOD_ERASE:
        result = await overlay.erase(dataset, targetDataset, resultDataset, overlayParameter)
        break
      case METHOD_INTERSECT:
        result = await overlay.intersect(dataset, targetDataset, resultDataset, overlayParameter)
        break
      case METHOD_IDENTITY:
        result = await overlay.identity(dataset, targetDataset, resultDataset, overlayParameter)
        break
      case METHOD_UNION:
        result = await overlay.union(dataset, targetDataset, resultDataset, overlayParameter)
        break
      case METHOD_UPDATE:
        result = await overlay.update(dataset, targetDataset, resultDataset, overlayParameter)
        break
      case METHOD_XOR:
        result = await overlay.xOR(dataset, targetDataset, resultDataset, overlayParameter)
        break
    }
    return {
      result,
