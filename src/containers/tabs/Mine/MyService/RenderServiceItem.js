...
export default class RenderServiceItem extends PureComponent{
  ...
  _downloadMapFile = async (mapTitle) => {
    ...
    let savePath = await Utility.appendingHomeDirectory(ConstPath.UserPath+onlineFileName);
  ...
  _navigator = async (mapUrl,restTitle) =>{
    ...
    if(Platform.OS === 'ios'){
      if(publishMap.indexOf(restTitle) === -1){
        let publish =await SOnlineService.changeServiceVisibility(restTitle,true);
