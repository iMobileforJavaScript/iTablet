  ...
  _register = async () => {
    try {
      ...
      if (typeof result === 'boolean' && result === true) {
        ...
        switch(result){
          case '手机号已注册':
            info=getLanguage(this.props.language).Prompt.PHIONE_HAS_BEEN_REGISTERED
            break
          case '昵称已存在':
            info=getLanguage(this.props.language).Prompt.NICKNAME_IS_EXISTS
            break
          case '短信验证码错误':
            info=getLanguage(this.props.language).Prompt.VERIFICATION_CODE_ERROR
            break
          case '邮箱已注册':
            info=getLanguage(this.props.language).Prompt.EMAIL_HAS_BEEN_REGISTERED
            break
          case '注册失败':
        ...
        <View style={styles.verifyCodeViewStyle}>
          ...
          <TouchableOpacity
            onPress={() => {
              ...
              Toast.show( getLanguage(this.props.language).Prompt.VERIFICATION_CODE_SENT)
              //'验证码已发送')
