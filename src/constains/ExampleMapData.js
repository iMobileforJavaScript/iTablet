import  ConstPath  from '../constains/ConstPath'
export default {
  testData_android: [
    { key: '数据可视化', path: ConstPath.SampleDataPath + '/hotMap/hotMap.smwu' },
    { key: 'GL地图瓦片', path: ConstPath.SampleDataPath + '/Changchun/Changchun.smwu' },
    { key: '影像叠加矢量地图', path: ConstPath.SampleDataPath + '/DOM/DOM.smwu' },
    { key: '三维场景', path: ConstPath.SampleDataPath + '/CBD_android/CBD_android.sxwu' },
    { key: '倾斜摄影', path: ConstPath.SampleDataPath + '/MaSai/MaSai.sxwu' },
  ],
  testData_ios: [
    { key: '数据可视化', path: ConstPath.SampleDataPath + '/hotMap/hotMap.smwu' },
    { key: 'GL地图瓦片', path: ConstPath.SampleDataPath + '/Changchun/Changchun.smwu' },
    { key: '影像叠加矢量地图', path: ConstPath.SampleDataPath + '/DOM/DOM.smwu' },
    { key: '三维场景', path: ConstPath.SampleDataPath + '/CBD_ios/CBD_ios.sxwu' },
    { key: '倾斜摄影', path: ConstPath.SampleDataPath + '/MaSai_ios/MaSai.sxwu' },
  ],
}